...
def get_model(data,_params, model_type):
    if model_type == "GCN":
        ...
        model = GATv2(data.num_node_features, _params['hidden_size'],_params['hidden_size'], dropout= _params['dropout_rate'])
    elif model_type == "GAT_V1":
        model = GATv1(data.num_node_features, _params['hidden_size'], _params['hidden_size'], dropout=_params['dropout_rate'])
    else:
...
def evaluate_metrics(model_name, model, _loader, device):
    ...
    with torch.no_grad():
        ...
        for graph, commit_id, index in _loader:
            ...
            # print('Out: {}, target: {}'.format(out.argmax(dim=1).cpu().detach().numpy(), target))
